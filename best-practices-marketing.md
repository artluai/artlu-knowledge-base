...
Lookup specific IDs from saved file:
```bash
python3 -c "
import json, sys
data = json.load(open('categories.json'))
targets = [10108, 11867, 12973]
for item in data['tasks'][0]['result']:
    if item['category_code'] in targets:
        print(item['category_code'], '-', item['category_name'])
"
```
...
**Limitations:** Does not support `filters` or `order_by` parameters.  
**Workaround:** Pull raw data and filter locally with Python.
...
#### 5. Keyword Ideas
**What it does:** Returns keyword ideas related to seed keywords, with CPC, volume, competition, and intent.  
**Supports filters:** Yes — can filter by CPC, competition, intent at API level.  
**Use for:** Drilling into a specific niche once identified.
...
3. Filter by CPC>$2, competition>0.1, transactional/commercial intent
